...
import { Trophy, Lock, KeyRound, ExternalLink } from 'lucide-react';

interface CombinationGameProps {
  gameId: string;
  isAdmin: boolean;
}

export function CombinationGame({ gameId, isAdmin }: CombinationGameProps) {
  ...
  if (loading) {
    return (
      <div className="text-center py-8">
        <div className="text-slate-400 animate-pulse">Loading combination terminal...</div>
      </div>
    );
  }
  ...
  if (game?.status === 'completed' && !hasWon) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-slate-600 font-medium">The combination has been cracked!</p>
        <p className="text-slate-400 text-sm mt-1 mb-5">
          Solved by {game.winnerTeamName ?? 'another team'}
        </p>
        <a
          href="/games/combination-game"
          target="_blank"
  ...
  if (hasWon) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-xl font-bold text-slate-800 mb-1">COMBINATION CRACKED!</h3>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-800/95 rounded-xl p-5 sm:p-6 text-center">
        <div className="w-14 h-14 rounded-full bg-cyan-900/30 flex items-center justify-center mx-auto mb-4">
          <KeyRound className="w-7 h-7 text-cyan-400" />
        </div>
        <h3 className="text-amber-400 font-bold text-lg mb-2">The Combination Awaits</h3>
        <p className="text-slate-400 text-sm max-w-md mx-auto mb-6 leading-relaxed">
          Walk down the path. Read the sign. Choose the right combination across all 4 stages.
        </p>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-cyan-700 hover:bg-cyan-600 text-white font-bold text-sm uppercase tracking-wider transition-colors shadow-lg"
        >
          <KeyRound className="w-4 h-4" />
          OPEN THE COMBINATION GAME
          <ExternalLink className="w-4 h-4" />
        </a>

        <p className="text-slate-500 text-xs mt-4">
          Crack the combination to discover the code, then enter it below.
        </p>
      </div>
      ...
      <div className="bg-slate-800/95 rounded-xl p-5 sm:p-6">
        ...
        <div className="flex gap-2 max-w-md mx-auto">
          <input
            ...
            placeholder="Enter the code you discovered..."
